...
for ((counter=0; counter <= 100; counter++)); do
  ...
  # run and process
  ${simgi_exe} irreversible_trimol.1.sgl >& /dev/null 

  a=$(tail -n 1 irreversible_trimol.1.dat | gawk ' { print $1 }')
  b=$(tail -n 1 irreversible_trimol.1.dat | gawk ' { print $2 }')
  c=$(tail -n 1 irreversible_trimol.1.dat | gawk ' { print $3 }')
  d=$(tail -n 1 irreversible_trimol.1.dat | gawk ' { print $4 }')
  ...
  # unlink
  rm -f irreversible_trimol.1.dat || return 1
