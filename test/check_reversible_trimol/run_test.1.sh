...
for ((counter=0; counter <= 200; counter++)); do
  ...
  # run and process
  ${simgi_exe} -s ${counter} reversible_trimol.1.sgl >& /dev/null 
  tail -n 2000 reversible_trimol.1.dat | ../test_helpers/average >> ${globalFile} || return 1

  # unlink
  rm -f reversible_trimol.1..dat || return 1
...
../test_helpers/check_deviation ${globalFile} 2.1986e-05 5e-4
status=$?
